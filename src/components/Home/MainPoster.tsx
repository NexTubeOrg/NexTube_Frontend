...
export const MainPoster = () => {
  ...
  const loadRandomVideoAsync = async () => {
    const result = (
      await http_api.get<IGetVideoResult>(
        `/api/video/getRandomVideo`,
      )
    ).data;
      ...
      <div className="h-[50vh] p-20">
        <div className="h-full flex items-center justify-between">
          <div className="flex z-[9997] flex-col min-[1000px]:w-[32em] min-[1364px]:w-[64em]">
            ...
            <div className="mb-2">
            <a href={'/video/watch/'+randomVideo?.id}><h2 className=" font-semibold text-primary  text-5xl">
                {randomVideo?.name}
              </h2> </a>
            </div>
            <div className="mb-2">
            <a href={'/channel/'+randomVideo?.creator?.userId}> <p className="font-semibold text-white text-3xl">
                {t('mainPoster.from') + "  " + randomVideo?.creator?.firstName + " " + randomVideo?.creator?.lastName}
              </p></a>
            </div>
          ...
          <a className="relative h-full w-full " href={'/video/watch/'+randomVideo?.id}>
